...
namespace Models {
	...
	void Torus::drawSolid(bool smooth) {
		...
		if (!gpuDataUploaded) {
			...
			glBufferData(GL_ARRAY_BUFFER,internalVertices.size()*sizeof(vec4),internalVertices.data(),GL_STATIC_DRAW);

			glBindBuffer(GL_ARRAY_BUFFER,normalsVbo);
			glBufferData(GL_ARRAY_BUFFER,internalFaceNormals.size()*sizeof(vec4),internalFaceNormals.data(),GL_STATIC_DRAW);

			glBindBuffer(GL_ARRAY_BUFFER,vertexNormalsVbo);
			glBufferData(GL_ARRAY_BUFFER,internalVertexNormals.size()*sizeof(vec4),internalVertexNormals.data(),GL_STATIC_DRAW);
